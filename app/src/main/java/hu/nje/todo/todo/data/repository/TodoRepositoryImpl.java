...
public class TodoRepositoryImpl implements TodoRepository {
    ...
    public void patchTodo(Long todoId, TodoUpdateRequest request,
            TodoCallback<TodoResponse> callback) {
        todoApi.patchTodos(todoId, request).enqueue(new Callback<>() {

            @Override
            public void onResponse(@NotNull Call<TodoResponse> call,
                    @NotNull Response<TodoResponse> response) {
                if (response.isSuccessful() && response.body() != null) {
                    callback.onSuccess(response.body());
                } else {
                    Log.e(TAG,
            ...
            @Override
            public void onFailure(@NotNull Call<TodoResponse> call, @NonNull Throwable throwable) {
                callback.onError("Network error: " + throwable.getMessage());
            }
        });
    }
