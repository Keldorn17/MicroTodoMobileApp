...
public class TodoEditorFragment extends Fragment {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        getParentFragmentManager().setFragmentResultListener("category_request",
                getViewLifecycleOwner(), (requestKey, result) -> {
                    ArrayList<String> updatedCategories = result.getStringArrayList("categories");
                    if (updatedCategories != null) {
                        viewModel.setCategories(new HashSet<>(updatedCategories));
                    }
                });
    ...
    private void setupClickListeners() {
        ...
        binding.btnSave.setOnClickListener(v -> {
            String title =
                    binding.etTitle.getText() != null ? binding.etTitle.getText().toString().trim()
                                                      : "";
    ...
    private void showDatePicker() {
        ...
        if (currentDt != null) {
            calendar.set(currentDt.getYear(), currentDt.getMonthValue() - 1,
    ...
    private void observeViewModel() {
        viewModel.getDeadline().observe(getViewLifecycleOwner(), dt -> {
            if (dt != null) {
                ...
                binding.btnDeadlineTime.setText(
        ...
        viewModel.isAccessDenied().observe(getViewLifecycleOwner(), denied -> {
            if (denied != null && denied) {
                Toast.makeText(requireContext(), "You have been removed from this Todo",
                        Toast.LENGTH_SHORT).show();
                Navigation.findNavController(binding.getRoot()).popBackStack();
            }
        });
    ...
    private TextView createCategoryPill(String category) {
        ...
        shape.setCornerRadius(TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 100,
                getResources().getDisplayMetrics()));
        shape.setStroke((int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 1,
                getResources().getDisplayMetrics()), secondaryColor);
